...
The main loop: user types, agent perceives, caches context, filters invalid state, presents options for human decision, remembers the session, consolidates into skills.

### Tool execution subpipe

```
Model response → Perceive (extract tool calls) → Cache (queue) → Filter (policy) → [human confirms] → Remember (results to history)
```
...
```
API call → Perceive (receive chunks) → Cache (buffer) → Filter (validate) → [inference] → Remember (add to history)
```
...
4. **Consolidate is top-level only.** The backward pass reads from the aggregate session store (top-level Remember) and writes to procedural memory. Sub-pipe-level Consolidate is a future extension.
